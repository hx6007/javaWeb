...
{  
    ...
    var tabidcounter = 0;
    var actions = {
        getMenu: ne.toUrl('web/user_menus')
    };
    ...
    { 
        ...
        ne.ajax({
            ...
            success: function (r)
            {
                ...
                {
                    ne.tips({ type: 2, content: r.Message || r.Data });
                    return;
                } else if (r.statusCode == "3") //系统级错误
                {
                    ne.tips({ type: 1, content: r.Message || r.Data });
                    return;
                }
                initMenus(r);
                initMenuEvent();
    ...
    function initMenus(allMenus)
    {
        ...
        $(topMenus).each(function (i, menu)
        {
            ...
            jtopmenu_item.find("img").attr("src", ne.toUrl(menu.MenuIcon));
            ...
            $(groups).each(function (gi, group) //包括分组的部分
            {
                ...
                $(items).each(function (i, submenu)
                {
                    ...
                    var url = ne.toUrl(submenu.MenuUrl);
            ...
            { 
                ...
                item_html = ne.templateRender(item_html, submenu);
                var item = $(item_html).appendTo(mainmenu); 
