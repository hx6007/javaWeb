...
if(parmParent.isHas()){
     ...
     $pb.mergeMap(parm,parmMap);
      $pb.Data.load(parm,tiketMap,"118","",function(data){
       				if(data.code==0){
                                        pb.cols=getCols(data.data[0]);
...
$("#panFrom1").html('<span id="'+saveFrom+'"></span>');
       pb_Pan=$("#"+saveFrom).ligerForm({
			...
			labelAlign:"right",
			space : 2,
			fields : [{"newline":false,"name":"Item","display":getDes('Item')}]
	});
	 var obj = {           
         };
	if(parmParent.isHas()){
...
var filedList = [{"isSort":"false","align":"left","width":100,"name":"Item","display":getDes('Item')},{"isSort":"false","editor":{"type":"text"},"align":"left","width":100,"name":"user_name","display":getDes("user_name")}];
var fiedList_result = $pb.findMap(filedList,"name");
        console.log(fiedList_result);
function getCols(m){
    var cols=[];
    for(var key in m){
        if(fiedList_result[key]) {
            console.log(fiedList_result[key][0]);
            cols.push(fiedList_result[key][0]);
        }else{
        cols.push({"isSort": "false", "align": "left", "width": 130, "name": key, "display": getDes(key)});
        }
    }
    return cols;
}
