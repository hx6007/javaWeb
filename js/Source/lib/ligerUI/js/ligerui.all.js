    ...
    $.ligerui.controls.CheckBoxList.ligerExtend($.ligerui.controls.Input, {
        ...
        refresh:function()
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                out.push("<td align='left'><input type='checkbox' name='" + name + "' value='" + val + "' id='" + id + "'/><label for='" + id + "'>" + txt + "</label></td>");
            }
            var yu=data.length%rowSize;
            if(yu!=0){
	            out.push("<td colspan='"+(rowSize-yu)+"'></td>");
            }
            if (appendRowStart) out.push('</tr>');
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        */
        ...
        deleteSelectedRow: function ()
        {
            ...
            if(notDel){
            	$.ligerDialog.error('有选择确认的信息，已经禁止删除！');
            }
        ...
        */
        ...
        _applyEditor: function (obj)
        {
            ...
            if (column.editor.type && p.editors[column.editor.type])
            {
                ...
                var editor = p.editors[column.editor.type],
                    ...
                    left = pc.left + pb.left + pv.left - (column.frozen?g.f.gridtablewidth:0),
        ...
        _onClick: function (e)
        {
            ...
            else if (src.row && g.enabledCheckbox()) //复选框选择行
            {
                ...
                var hasEdit=true;
                if(src.data.hasOwnProperty("pbOk")){
                	hasEdit=(src.data.pbOk=="1")?false:true;
                }
                if (!src.checkbox && src.cell && p.enabledEdit && p.clickToEdit && hasEdit)
                {
            ...
            else if (src.row && !g.enabledCheckbox() && p.selectable != false)
            {
                ...
                if (src.cell && p.enabledEdit && p.clickToEdit && hasEdit)
                {
                    g._applyEditor(src.cell);
                }
...
(function ($)
{
    ...
    $.ligerui.controls.Menu.ligerExtend($.ligerui.core.UIComponent, {
        ...
        },showParent:function(menu){
        	var g = this;
        	if (menu == undefined) menu = g.menu;
        	if(menu.tongjiu){
        		g.showParent(menu.tongjiu);
        	}
        	menu.show();
        },
        addItem: function (item, menu)
        {
            ...
            if (item.children)
            {
                ...
                newmenu.hover(function(){
            	    if(!newmenu.tongjiu){
            	      newmenu.tongjiu=g.shanMenu;
            	    }
            	    g.showParent(newmenu);
                },function(){
                	 g.shanMenu=newmenu;
	   	        	 g.hideAllSubMenu(newmenu);
                });
            ...
            ditem.hover(function (){
                ...
                $(menu.items).hover(null,function(){//添加整个区域离开事件
                	g.hide();
                });
            }, function (){
            	 g.shanMenu=menu;
    ...
    $.ligerui.controls.RadioList.ligerExtend($.ligerui.controls.Input, {
        ...
        refresh: function ()
        {
            ...
            for (var i = 0; i < data.length; i++)
            {
                ...
                out.push("<td align='left'><input type='radio' name='" + name + "' value='" + val + "' id='" + id + "'/><label for='" + id + "'>" + txt + "</label></td>");
            }
            var yu=data.length%rowSize;
            if(yu!=0){
	            out.push("<td colspan='"+(rowSize-yu)+"'></td>");
